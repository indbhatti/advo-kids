...
import { correct, getCurrentQuestionNumber, getQuestion } from '@/server-actions/serveractions'
...
export default function QuizF({ user, storylineNumber }: { user: SimpleUser, storylineNumber: number }) {
  ...
  const setQ = async () => {
    const currentQuestionNumberGot = await getCurrentQuestionNumber(user._id, storylineNumber);

    if (currentQuestionNumberGot) {
      const question: SimpleQuestion | null = await getQuestion(user.language, storylineNumber, currentQuestionNumberGot);
      if (question) {
  ...
  if (currentQuestion != undefined) {
    const handleClick = async () => {
      if (selectedOption === currentQuestion.answer) {
        try {
          const response = await correct(user._id, currentQuestion.storylineNumber, currentQuestion.questionNumber);
    ...
    return (
      <div className="container bg-opacity-30 bg-gray-300 p-1 rounded-3xl">
        ...
        <div className="m-5">
          ...
          />
          <h1 className="text-5xl mt-5 font-sans font-extrabold">{user.language === "English" ? "Question" : hindi.quesiton} {currentQuestion.questionNumber}:</h1>
          <p className="text-xl my-5 font-sans font-bold">{currentQuestion.questionStatement}</p>
